...
export default async function handler(req, res) {
  ...
  for (const k of ["GAS_URL", "GAS_TOKEN", "OWNER_NAME", "ADMIN_PIN", "TICK_KEY", "WORKS_CLIENT_ID", "WORKS_CLIENT_SECRET", "WORKS_SERVICE_ACCOUNT", "WORKS_PRIVATE_KEY", "WORKS_BOT_ID", "WORKS_BOT_SECRET", "IMAP_HOST", "IMAP_USER", "IMAP_PASS", "SMTP_HOST", "SMTP_USER", "SMTP_PASS", "OWNER_EMAIL", "GEMINI_API_KEY", "ANTHROPIC_API_KEY"]) {
  ...
  if (env.SMTP_ENABLED) { try { await new Mailer().verify(); r.smtp = { ok: true, from: env.SMTP_USER, to: env.OWNER_EMAIL || env.SMTP_USER }; } catch (e) { r.smtp = { ok: false, error: e.message }; } }
  else r.smtp = { ok: false, error: "SMTP 미설정 — 브리핑 메일이 오지 않습니다 (웹페이지에서는 그대로 볼 수 있음)" };
  if (env.IMAP_ENABLED) { try { r.mail = await testMailLogin(); } catch (e) { r.mail = { ok: false, error: e.message }; } }
  else r.mail = { ok: false, error: "IMAP 미설정 — 받은편지함에서 할 일을 모으지 않습니다 (선택 기능)" };
