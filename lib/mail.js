...
export async function fetchNewMail({ lastUid = 0, limit = 40, sinceDays = 3, cfg = env } = {}) {
  ...
  const client = new ImapFlow({
    host: cfg.MAIL_HOST, port: cfg.MAIL_PORT, secure: true, logger: false,
...
export async function testMailLogin(cfg = env) {
  const { ImapFlow } = await import("imapflow");
  const client = new ImapFlow({ host: cfg.MAIL_HOST, port: cfg.MAIL_PORT, secure: true, logger: false, auth: { user: cfg.MAIL_USER, pass: cfg.MAIL_PASS } });
  await client.connect();
