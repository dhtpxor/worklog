...
export async function runTick({ gas, works, mailer, cal = null, cfg, fetchMail, now = new Date(), force = "" }) {
  ...
  // 1) 메일 수집 (매 호출)
  if ((cfg.IMAP_ENABLED && fetchMail) || force === "mail") {
    try {
      ...
      for (const msg of messages) {
        ...
        const myAddrs = [cfg.IMAP_USER, cfg.SMTP_USER, cfg.OWNER_EMAIL].filter(Boolean).map((x) => x.toLowerCase());
        const senderIsMe = myAddrs.includes(fromAddr);
  ...
  if (!notifier.channels.length) { out.done.push("알림 채널 없음(메일 발송·네이버웍스 둘 다 미설정) — 웹페이지에서 확인"); return out; }
